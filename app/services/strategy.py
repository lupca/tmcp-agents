...
async def marketing_strategy_event_generator(
    ...
    try:
        ...
        async def _fetch_and_parse(collection: str, record_id: str) -> dict:
            result = await execute_mcp_tool("get_record", _mcp_args(collection, record_id))
            return json.loads(result.content[0].text)

        # Emit all status events sequentially before parallelizing
        ...
        tasks = [
            _fetch_and_parse("brand_identities", brand_identity_id),
            _fetch_and_parse("customer_personas", customer_profile_id)
        ]
        if product_id:
            tasks.append(_fetch_and_parse("products_services", product_id))
